...
impl TextInput {
    ...
    fn insert(
        ...
        // Normalise line endings before the text lands in the document and the
        // undo stack: parley counts CR and LF as two independent line breaks,
        // so a pasted `\r\n` (Windows clipboard) would render every row with a
        // stray empty line, and its `\r` byte keeps desyncing the LF-only line
        // model (gutter rows, highlight bands) built around the document.
        // Meatshell vendor patch (drop when upgrading Slint to a release that
        // normalises this upstream).
        let text_to_insert = if text_to_insert.contains('\r') {
            alloc::borrow::Cow::Owned(
                text_to_insert.replace("\r\n", "\n").replace('\r', "\n"),
            )
        } else {
            alloc::borrow::Cow::Borrowed(text_to_insert)
        };
        ...
        let mut inserted_text: SharedString = text_to_insert.as_ref().into();
        if text_to_insert.contains('\n') && self.single_line() {
            inserted_text = text_to_insert.as_ref().replace('\n', " ").into();
            text.insert_str(cursor_pos, &inserted_text);
        } else {
            text.insert_str(cursor_pos, text_to_insert.as_ref());
        }
